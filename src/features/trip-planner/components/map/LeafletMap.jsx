...
const LeafletMap = ({
  ...
  const tileThemeRef = useRef('');
  const isProgrammaticMoveRef = useRef(false);
  const isNavigatingRef = useRef(isNavigating);
  const lastRouteParamsRef = useRef({ spotsKey: '', showTravelRoute: false });
  ...
  const getFollowTarget = useCallback((location, heading, useForwardOffset = false) => {
        ...
        const userPulseHtml = `
          <div class="relative flex flex-col items-center justify-center" style="margin-top: -15px;">
            <div class="relative flex items-center justify-center h-10 w-10 select-none">
              ...
              <div class="absolute h-8 w-8 rounded-full bg-gradient-to-tr from-blue-600 to-cyan-500 border-2 border-white shadow-xl flex items-center justify-center relative z-10 transition-transform duration-300" style="${arrowRotationStyle}">
                <!-- Glowing inner pulse -->
                ...
                <svg class="w-4 h-4 text-white filter drop-shadow-[0_1px_3px_rgba(0,0,0,0.3)]" fill="currentColor" viewBox="0 0 24 24">
