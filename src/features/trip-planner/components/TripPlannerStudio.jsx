...
export default function TripPlannerStudio({ prefill }) {
  ...
  const getMapEmbedUrl = () => {
    ...
    if (showTravelRoute) {
      ...
      const spots = displaySlots.map(s => s.spot);

      if (spots.length > 0) {
        const startLat = activeAcc ? activeAcc.lat : spots[0].lat;
        const startLng = activeAcc ? activeAcc.lng : spots[0].lng;

        const lastSpot = spots[spots.length - 1];
        const destLat = lastSpot.lat;
        const destLng = lastSpot.lng;

        let daddr = '';
        if (spots.length > 1) {
          const intermediateSpots = spots.slice(0, spots.length - 1);
          daddr = intermediateSpots.map(s => `${s.lat},${s.lng}`).join('+to:') + `+to:${destLat},${destLng}`;
        } else {
          daddr = `${destLat},${destLng}`;
        }

        return `https://maps.google.com/maps?saddr=${startLat},${startLng}&daddr=${daddr}&t=h&z=15&output=embed`;
      }
    }
    ...
    return `https://maps.google.com/maps?q=${destLat},${destLng}&z=16&t=h&output=embed`;
  };
              ...
              <div className="w-full flex flex-col gap-6 animate-fade-in">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-200 pb-3 overflow-x-auto select-none">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300 cursor-pointer flex items-center gap-1.5 border shadow-sm ${
                      showTravelRoute
            ...
            <div className="bg-heritage-amber text-white px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
              ...
              <div className="flex sm:hidden bg-white/20 p-1 rounded-xl border border-white/10 w-fit self-center">
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all border-none ${activeModalView === 'map' ? 'bg-white text-heritage-amber shadow-sm' : 'text-white hover:bg-white/5 bg-transparent'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all border-none ${activeModalView === 'details' ? 'bg-white text-heritage-amber shadow-sm' : 'text-white hover:bg-white/5 bg-transparent'
                    }`}
                >
